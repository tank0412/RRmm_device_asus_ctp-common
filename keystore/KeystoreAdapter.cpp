...
static int keymaster_device_open(const hw_module_t* module, const char* name, hw_device_t** device) {
    int rv;
    rv = hw_get_module_by_class("keystore", "vendor", (const hw_module_t **)&gVendorModule);

    if (rv) {
        ALOGE("failed to open vendor keystore module");
        return rv;
    } else {
        return gVendorModule->common.methods->open((const hw_module_t*)gVendorModule, name, device);
    }
}
...
struct keystore_module HAL_MODULE_INFO_SYM = {
    .common = {
         ...
         .name = "Clovertrail Keystore Adapter",
